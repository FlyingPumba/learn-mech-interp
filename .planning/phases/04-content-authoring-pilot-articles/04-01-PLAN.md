...
must_haves:
  truths:
    ...
    - "Image passthrough copy is configured for all topic image directories"
...
<task type="auto">
  ...
  <action>
    ...
    **3. Expand references.json with all citations needed for 3 pilot articles:**
    ...
    That gives us citations for all 3 pilot articles. The existing `elhage2022toy` entry covers superposition. Total: 11 entries.
  </action>
  <verify>
    Run `npx @11ty/eleventy --serve` and confirm the site builds with no errors. Check that references.json is valid JSON with 11 entries.
  </verify>
  <done>
    eleventy.config.js has image passthrough, article.njk renders prerequisites, references.json has 11 entries covering all 3 pilot articles.
  </done>
</task>
