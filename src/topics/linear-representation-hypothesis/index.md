...
**The Wolf and Coke Can Problem.** Early vision interpretability work discovered a striking example: a single neuron that fired strongly for both wolves and Coca-Cola cans {% cite "olah2020zoom" %}. Why would these unrelated concepts share a neuron? The likely explanation: the training data never contained images with both wolves and cans together. If the two concepts never co-occur, the model can safely reuse the same neuron for both, freeing up parameters for other purposes.

From a learning perspective, this is clever resource allocation. From an interpretability perspective, it is a nightmare. If you think you have found the "wolf neuron" and test it on pictures of wolves, it fires. Success! But you do not know it also fires for cans. You might try to ablate this "wolf neuron" to see if wolf detection breaks, and find nothing happens, because on your test images the neuron was actually responding to something else entirely.
