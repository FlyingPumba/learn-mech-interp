...
**Faithfulness** asks: does the circuit reproduce the full model's behavior? The test is to run the model with only the circuit components active (ablating everything else) and compare the output to the full model. The IOI circuit recovers approximately 87% of the full model's logit difference. This is good but not perfect -- the remaining 13% comes from components outside the circuit, including MLPs and minor attention head contributions. Later work revealed that faithfulness can often be more informative than circuit overlap {% cite "hanna2024faithfulness" %}, but samplewise variance complicates its interpretation {% cite "miller2024transformer" %}{% cite "wu2026variance" %}. For more details, see [refined attribution methods](/topics/refined-attribution-methods/).
